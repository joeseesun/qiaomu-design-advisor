...
9. 预览页提供三拨盘滑块、选择确认弹层和调整建议输入框；确认后回传方向 + 拨盘 + 建议
10. 预览页没有 60 秒自动推进或自动确认；必须用户点选/键盘选择并确认，或在对话中明确选择
11. 最终回复必须提供预览目录、`index.html` 的可点击本地路径和本地预览 URL（若已启动）
12. 交付前必须运行最低限度检查：文件存在、HTML 可解析、包含 4 个方向名、包含「推荐」
